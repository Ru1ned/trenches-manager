...
function activateWhiteFlag(tribeId) {
  ...
  // Prevent duplicate white flag events
  if (anchor.whiteFlag && anchor.whiteFlag.active && anchor.whiteFlag.expiresAt > now()) {
    if (_ctx) _ctx.log(`[raidZones] white flag already active for tribe ${tribeId}, skipping`, 'debug');
    return;
  }
