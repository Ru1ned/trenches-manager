...
function init(ctx) {
  ...
  _cleanupInterval = setInterval(() => { cleanPairCooldowns(); }, 120000);
...
module.exports = {
  ...
  syncRaidData,
  flush,
  // Read-only accessors for stats API (external mod / leaderboard consumption)
  _getPlayerBuf: (eosId) => _playerBuf.get(eosId) || null,
  _getPlayerMap: () => _playerBuf,
  _getTribeMap: () => _tribeBuf,
};
